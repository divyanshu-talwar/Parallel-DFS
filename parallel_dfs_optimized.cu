...
__global__ void subgraph_size(int* zeta, int* zeta_tilde, graph* dataset_graph) {
	...
	memcpy(q, dataset_graph->leaves, sizeof(int) * dataset_graph->vertices);
	...
	while(true) {
		...
		memset(c, false, dataset_graph->vertices * sizeof(bool));
		bool global_check = false;
		
		for(int i = 0; dataset_graph->vertices; i++) {
			if( q[i] ) {
...
__global__ void dag_to_dt(int* depth, int* parent, char **global_path, graph* dataset_graph) {
